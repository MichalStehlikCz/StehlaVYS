 ...
 * @author stehlik
 */
public class JsonbDtNameDeserializer implements JsonbDeserializer<DtName> {
    @Override
    public DtName deserialize(JsonParser jsonParser, DeserializationContext deserializationContext, Type type) {
        DtName dt = null;
        ...
        if (event == JsonParser.Event.VALUE_STRING || event==JsonParser.Event.VALUE_NUMBER) {
            dt=new DtName(jsonParser.getString());
        } else {
